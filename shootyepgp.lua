...
local bidlink = {
  ...
  ["os"] = "|cff0099FF|Hshootybid:2:$ML|h[Offspec]|h|r",
}
local options
do
  for i=1,40 do
...
function sepgp:SetItemRef(link, name, button)
  ...
  -- =========================
  -- NOWA LOGIKA BID”W (AddonMessage ñ VANILLA SAFE)
  -- =========================
...
function sepgp:refreshPRTablets()
  -- Od≈õwie≈º standings, zawsze istnieje
  if sepgp_standings and sepgp_standings.Refresh then
      sepgp_standings:Refresh()
  end

  -- Od≈õwie≈º bids tylko je≈õli modu≈Ç istnieje i ma metodƒô Refresh
...
function sepgp:decay_epgp_v3()
  ...
  local addonMsg = string.format(
    "ALL;DECAY;%s",
    (1 - (sepgp_decay or sepgp.VARS.decay)) * 100
  )
...
function sepgp:capcalc(ep, gp, gain)
  ...
  local pr_decay =
    tonumber(string.format("%.03f", pr)) -
    tonumber(string.format("%.03f", ep_decayed / gp_decayed))
...
function sepgp:captureAddonBid(prefix, message, channel, sender)
  ...
  -- TERAZ rÍcznie dodajemy bid (BEZ whisperÛw)
...
do
  ...
  UIDropDownMenu_AddButton = function(info, level)
    ...
    then
      ...
      -- Je≈õli to Self ‚Üí dodaj natychmiast
      if info.text == playerName then
        original_AddButton(info, level)
        return
      end

      -- Resztƒô op√≥≈∫nij o jednƒÖ klatkƒô
