  ...
  if not T:IsRegistered("sepgp_loot") then
    T:Register("sepgp_loot",
      ...
      "menu", function()
        ...
        -- Eksport historii lootów
        D:AddLine(
          "text", C:Green("Export loot history"),
          "tooltipText", "Export loot history to text",
          "func", function()
            sepgp_loot:ExportLootHistory()
          end
        )
      end
    )
...
function sepgp_loot:OnTooltipUpdate()
  ...
  for i = 1, table.getn(t) do
    ...
    local gpValue = 0
    if action == "MS" or action == sepgp.VARS.msgp then
      gpValue = price or 0
    elseif action == "OS" or action == sepgp.VARS.osgp then
      gpValue = off_price or 0
    end

    local actionText = string.format("%s | %d GP", action, gpValue)
...
-- ====== ExportLootHistory ======
function sepgp_loot:ExportLootHistory()
  local t = self:BuildLootTable()
  if not t or table.getn(t) == 0 then return end

  local lines = {}
  table.insert(lines, "Time\tPlayer\tItem\tAction\tGP")

  for i = 1, table.getn(t) do
    local ts, player, _, item, _, price, off_price, action = unpack(t[i])
    ...
    local cleanItem = item:gsub("|c%x%x%x%x%x%x%x%x", ""):gsub("|r", "")
  ...
  sepgp:ShowExportFrame(table.concat(lines, "\n"))
end
